...
try {
    ...
    # 使用 robocopy 以提高複製速度並防範 Windows 260 字元路徑限制 (MAX_PATH)
    & robocopy.exe "$srcSitePackages" "$dstSitePackages" /E /NFL /NDL /NP /MT:8 | Out-Null
    if ($LASTEXITCODE -ge 8) {
        throw "robocopy copy site-packages failed with code $LASTEXITCODE"
    }
