...
> **這份文件重點在「介面怎麼看、功能怎麼用」。**  
> 如果你還在處理安裝環境、依賴套件或 CUDA，請先看根目錄的 `README.md`。
...
| 情境 | 啟動方式 |
|------|------|
| **打包發佈版** | 解壓後直接執行 `Stream Translator.exe` |
| **原始碼版** | 在專案根目錄執行 `cd app` → `python main.py` |

### 0.2 啟動前檢查清單

- 本專案以 **NVIDIA CUDA GPU** 為主，不建議以 CPU 模式使用。
- 第一次啟動 Qwen3-ASR / faster-whisper 時，可能會自動下載模型。
- 若要使用本地 LLM，請先準備好 `llama/` 執行環境與模型。
- 若首頁出現 ffmpeg 警告，代表 ffmpeg 尚未放到正確位置或未加入 PATH。
...
- **想先跑起來**：先看 [1. 三分鐘快速上手](#1-三分鐘快速上手)
- **想認識首頁和設定頁**：看 [2. 介面頁面說明](#2-介面頁面說明)
- **想照情境操作**：看 [3. 常見工作流程](#3-常見工作流程)
- **想研究硬體與模型搭配**：看 [2.5 ASR 顯卡與翻譯模型建議](#25-asr-顯卡與翻譯模型建議)
- **想查安裝與依賴**：請回到根目錄 `README.md`

---

## 1. 三分鐘快速上手

如果你第一次使用，不需要先把所有設定都研究完。先照下面流程跑通一次就好：
...
3. 先確認三個區塊：
   - **Input**：音訊來源型態
   - **Transcription**：ASR 後端與語言
   - **Translation**：翻譯後端、模型、目標語言
4. 回到 **Home**
5. 選擇音訊來源，填入網址或選裝置
6. 點 **Start**
7. 等待模型載入後，字幕會出現在主畫面與浮動字幕視窗
...
| 區塊 | 先看什麼 | 用途 |
|------|------|------|
| **Input** | URL / 裝置 / Cookies | 決定音訊從哪裡來 |
| **Transcription** | ASR 後端、模型、語言 | 決定辨識速度與品質 |
| **Translation** | 翻譯後端、目標語言、Base URL | 決定字幕翻譯怎麼跑 |

其餘像 VAD、術語表、輸出格式、字幕樣式，都可以等主流程跑通後再慢慢調。
...
Settings 的內容很多，但第一次使用不用全部打開。建議優先順序如下：

| 優先度 | 先調哪些 | 為什麼 |
|------|------|------|
| **最高** | Input / Transcription / Translation | 這三區決定能不能正常開始辨識與翻譯 |
| **中等** | Audio Slicing & VAD / Output / Server | 影響延遲、輸出格式與字幕分享 |
| **進階** | Terminology / Llama Settings / 細部參數 | 針對特定場景再調整 |
...
> 參數量來源：HuggingFace 模型頁；VRAM 來源：faster-whisper 官方 benchmark + 模型權重估算。
