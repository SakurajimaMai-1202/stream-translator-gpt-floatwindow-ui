...
from .audio_transcriber import OpenaiWhisper, FasterWhisper, SimulStreaming, RemoteOpenaiTranscriber, Qwen3ASRTranscriber, HFTranscriber, NemoASRTranscriber, SenseVoiceTranscriber, FunASRNanoTranscriber
...
def main(url, **kwargs):
    ...
    use_fun_asr = kwargs.get('use_fun_asr', False)
    fun_asr_model = kwargs.get('fun_asr_model') or 'FunAudioLLM/Fun-ASR-Nano-2512'
    fun_asr_device = kwargs.get('fun_asr_device') or 'auto'
    ...
    with ThreadPoolExecutor() as executor:
        ...
        def init_transcriber():
            ...
            if use_qwen3_asr:
                ...
                resolved_device = resolve_qwen3_device_map(
                    torch_module=torch,
                    requested_device_map=fun_asr_device,
                    runtime_profile=runtime_profile,
                    device_policy=runtime_device_policy,
                    allow_integrated_gpu=runtime_allow_integrated_gpu,
                )
                print(f'{INFO}Fun-ASR Nano device resolved: {resolved_device} '
                      f'(profile={runtime_profile}, policy={runtime_device_policy})')
                return FunASRNanoTranscriber(model=fun_asr_model, language=language,
                                             device=resolved_device, **common_args)
            elif use_hf_asr:
                ...
                print(f'{INFO}NVIDIA Parakeet device resolved: {resolved_device} '
                      f'(profile={runtime_profile}, policy={runtime_device_policy})')
                return NemoASRTranscriber(model=nemo_asr_model or model,
                                          language=language,
                                          proxy=processing_proxy,
                                          device=resolved_device,
                                          decoding=nemo_asr_decoding or 'tdt',
...
def cli():
    ...
    parser.add_argument('--nemo_asr_model', type=str, default='nvidia/parakeet-tdt_ctc-0.6b-ja', help='NVIDIA Parakeet model name.')
    parser.add_argument('--nemo_asr_device', type=str, default='auto', help='Device used when running NeMo ASR.')
    parser.add_argument('--nemo_asr_decoding', type=str, choices=['tdt', 'ctc'], default='tdt', help='Decoder head for hybrid NVIDIA Parakeet models.')
    parser.add_argument('--nemo_asr_dtype', type=str, choices=['auto', 'bfloat16', 'float16', 'float32'], default='bfloat16', help='Dtype for NVIDIA Parakeet.')
    ...
    parser.add_argument('--use_fun_asr', action='store_true', help='Use Fun-ASR Nano through FunASR.')
    parser.add_argument('--fun_asr_model', type=str, default='FunAudioLLM/Fun-ASR-Nano-2512', help='Fun-ASR Nano model repo id.')
    parser.add_argument('--fun_asr_device', type=str, default='auto', help='Device used when running Fun-ASR Nano.')
