...
建議從 GitHub Release 依照硬體下載對應的 v1.3.6 完整包，合併分割檔後解壓執行：
...
| SHA256 | `SHA256SUMS-v1.3.6.txt` | 驗證下載與合併後檔案完整性 |
...
v1.3.6 是目前建議使用的版本，重點改善直播切段、翻譯模型策略、原文與譯文成對提交，以及每句字幕的處理延遲顯示。v1.3.2 與 v1.3.3 已知存在設定頁與轉錄選項相關問題，請直接升級 v1.3.6。

- v1.3.2：不建議繼續使用，請升級 v1.3.6。
- v1.3.3：包含部分修正，但打包後仍可能在設定頁點選轉錄/ASR 選項時凍結，請升級 v1.3.6。
...
也可以使用 7-Zip 或其他支援分割檔的工具，從 `.part01` 開始合併/解壓。合併後請依照 `SHA256SUMS-v1.3.6.txt` 驗證檔案完整性。
