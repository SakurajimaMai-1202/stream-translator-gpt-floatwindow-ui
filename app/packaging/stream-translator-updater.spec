...
a = Analysis([str(app_dir / 'updater.py')], pathex=[str(app_dir)], datas=[(str(app_dir/'app_icon.ico'),'.')], hiddenimports=['yaml'], hookspath=[], hooksconfig={}, runtime_hooks=[str(app_dir / 'packaging' / 'pyi_rth_prefer_bundled_qt.py')], excludes=['PyQt6.QtWebEngineCore','PyQt6.QtWebEngineWidgets'], noarchive=False, optimize=0)
pyz = PYZ(a.pure)
