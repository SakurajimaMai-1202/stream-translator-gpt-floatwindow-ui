...
以下以 `Qwen/Qwen3-ASR-1.7B` 同時搭配本地翻譯模型為基準。實際顯存會受到 context 長度、KV cache、GPU offload、llama.cpp / LM Studio / Ollama 設定影響，表格用來做選型方向。

| 顯卡 | 推薦組合 | 評價 | 適合情境 |
|------|----------|------|----------|
| 8GB | Qwen3-ASR-0.6B + Hy-MT2-1.8B Q4_K_M | 最穩 | 低延遲、保守配置 |
| 12GB 穩定 | Qwen3-ASR-1.7B + Hy-MT2-1.8B Q4_K_M | 很穩 | 多語翻譯、長時間直播 |
| 12GB 品質 | Qwen3-ASR-1.7B + Hy-MT2-7B Q4_K_M | 可用但偏緊 | 多語翻譯品質優先 |
| 12GB 泛用 | Qwen3-ASR-1.7B + Gemma 4 E4B Q4 | 可用但偏緊 | 泛用翻譯、速度與品質平衡 |
| 12GB 日文 | Qwen3-ASR-1.7B 或 JA fine-tune + Sakura 7B IQ4_XS | 可用 | 日文、Galgame、輕小說語氣 |
| 16GB+ 日文品質 | Qwen3-ASR-1.7B + Sakura 14B IQ4 / Q4 | 推薦 16GB 以上 | 日中品質優先 |
| 16GB+ 多語品質 | Qwen3-ASR-1.7B + Hy-MT2-7B Q6_K | 推薦 16GB 以上 | 多語翻譯品質優先 |

12GB 顯卡的甜蜜點是 **Qwen3-ASR-1.7B + 4B～7B Q4 翻譯模型**。其中 Hy-MT2-7B Q4_K_M 與 Gemma 4 E4B Q4 是比較值得優先嘗試的組合；若要長時間穩定直播，Hy-MT2-1.8B Q4_K_M 會更保守。

Sakura 系列適合日文到中文、Galgame 與輕小說風格翻譯，但多數 Sakura 模型採非商用授權，公開發布或商業使用前請先確認授權。
