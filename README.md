...
[下載最新版](https://github.com/SakurajimaMai-1202/stream-translator-gpt-floatwindow-ui/releases/latest) · [v1.4.3 更新說明](app/docs/RELEASE_NOTES_v1.4.3_zh-TW.md) · [回報問題](https://github.com/SakurajimaMai-1202/stream-translator-gpt-floatwindow-ui/issues)
...
![Release](https://img.shields.io/badge/release-v1.4.3-blue)
...
完整內容請見 [v1.4.3 更新說明](app/docs/RELEASE_NOTES_v1.4.3_zh-TW.md)。

## 下載：先選對執行版本

請從 [GitHub Releases v1.4.3](https://github.com/SakurajimaMai-1202/stream-translator-gpt-floatwindow-ui/releases/tag/v1.4.3) 下載。GitHub 自動提供的 `Source code (zip)` 不是可直接執行的 Windows 完整包。
...
3. 以 `SHA256SUMS-v1.4.3.txt` 驗證合併後的 ZIP。
...
CUDA／ROCm 使用者若缺少 CPU ASR sidecar，可在「ASR 模型管理」內安裝，或下載 `StreamTranslator-CPU-ASR-Sidecar-v1.4.3.zip`。Sidecar 只含 CPU ASR Runtime，模型權重仍需另外下載。

v1.4.3 的 Full package 不包含 `llama` 資料夾。需要本機翻譯時，請在「LLM 模型管理」另外下載 GGUF 與相符的 llama.cpp Runtime。
...
- v1.4.3 Full package 內含 Node.js 22+，供 yt-dlp 處理需要 JavaScript Runtime 的來源。
...
# 快速驗證
.\build_all_profiles.ps1 -Version 1.4.3 -Mode Quick -ReuseRuntimeCache
...
正式資產輸出至 `app/release-v1.4.3-assets/`，包含 App Update、Full package 分割檔、manifest 與 SHA-256 清單。
