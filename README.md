...
> Main 分支需要 Windows 10/11 與 NVIDIA CUDA GPU。不提供 CPU-only 模式。

> ROCm 分支說明：`codex/rocm-asr` 是 AMD ROCm 實驗分支，目標只驗證 **Qwen3-ASR + VAD**。此分支需要可用的 AMD ROCm PyTorch 環境，不承諾本地 LLM 加速，也移除 Faster-Whisper、SimulStreaming 與 OpenAI Whisper API 等非 ROCm 本地 ASR 主線。
...
第一次使用 Qwen3-ASR 時，模型可能需要下載或載入，等待時間會比較長。
...
| 語音辨識 | ROCm 分支僅保留 Qwen3-ASR、Qwen3-ASR JA fine-tune |
...
| 顯卡 | 推薦組合 | 評價 | 適合情境 |
|------|----------|------|----------|
| 6GB | Qwen3-ASR-0.6B + API 翻譯 | 穩 | 入門、低延遲 |
...
| ROCm experimental | 此分支不提供 Faster-Whisper、SimulStreaming 或 OpenAI Whisper API ASR |
...
第一次使用模型時可能需要下載或載入權重。Qwen3-ASR 模型大小從數 GB 不等，請確認網路和磁碟空間足夠。
...
ROCm 分支僅保留 Qwen3-ASR。若出現重複輸出，優先調整 VAD 閾值、最短語音長度與音訊來源音量。
