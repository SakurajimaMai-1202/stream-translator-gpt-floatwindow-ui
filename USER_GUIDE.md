...
| **ffmpeg** | 提供音訊轉碼與處理；需在 PATH 或專案同層目錄 |
| **Node.js / npm** | 用來建構 `app/frontend/` |
| **llama-server.exe / DLL** | 只有要使用本地 LLM 時才需要 |
...
**Q：`requirements.txt` 是不是已經把所有東西都準備好？**  
不是。它只會處理 **Python 套件**。像 CUDA、cuDNN、NVIDIA Driver、ffmpeg、Node.js、llama-server 都需要另外準備。現在這份文件已把界線分清楚。

**Q：Qwen3-ASR 為什麼特別容易卡在環境？**  
...
**Q：ffmpeg 警告怎麼處理？**  
將 ffmpeg.exe 加入系統 PATH，或將 ffmpeg 資料夾放在專案同層目錄並命名為 `ffmpeg-8.1-essentials_build/`。
...
**Q：YouTube 讀取失敗？**  
部分影片或頻道需要提供 cookies。在瀏覽器安裝 「Get cookies.txt LOCALLY」擴充套件匯出 `cookies.txt`，在 Settings > Input > Cookies 填入路徑。

**Q：辨識結果有大量重複文字？**  
可在 Settings > Transcription > Whisper Filters 確認 `repetition_filter` 已啟用。Qwen3-ASR 通常不需要此過濾器。

**Q：字幕分享無法存取？**  
確認防火牆允許 Public Port（預設 8765）的連入連線。區網內其他裝置用 `http://[本機IP]:8765` 存取。

**Q：目前這個專案對 Python 版本有沒有建議？**  
...
如需，我可以再幫你產出第二版「**圖文版操作手冊**」（含每個頁面建議截圖位置與標註文案）。
