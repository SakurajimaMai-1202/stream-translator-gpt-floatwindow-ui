...
> ROCm 分支說明：`rocm-asr` 是 AMD ROCm 實驗分支，目標只驗證 **Qwen3-ASR + VAD**。此分支需要可用的 AMD ROCm PyTorch 環境，不承諾本地 LLM 加速，也移除 Faster-Whisper、SimulStreaming 與 OpenAI Whisper API 等非 ROCm 本地 ASR 主線。
