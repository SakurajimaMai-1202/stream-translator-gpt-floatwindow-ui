...
$pythonExe = $null
$candidateProbe = if ($Profile -eq "cpu") {
    "import sherpa_onnx, sys; print(sys.executable)"
} else {
...
foreach ($candidate in $pythonCandidates) {
    ...
    & $candidate -c $candidateProbe *> $null
...
profile = '$Profile'
required = ['opencc', 'sherpa_onnx', 'numpy', 'scipy', 'omnivad', 'stream_translator_gpt.main'] if profile == 'cpu' else ['opencc', 'qwen_asr', 'funasr', 'torchaudio', 'omnivad']
if profile == 'cuda': required.extend(['faster_whisper', 'whisper', 'nemo.collections.asr.models'])
